...
function SectionHeader({ label }: { label: string }) {
  return (
    <div className="flex w-full flex-col gap-3">
      <p className="text-[14px] font-medium uppercase leading-none tracking-[0.08em] text-[#65686B]">
        {label}
      </p>
...
function SummaryRow({
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
      <p className="shrink-0 text-[15px] font-normal leading-[1.25] text-[#7E8185]">
        {label}
      </p>
      <div className="flex items-center gap-2 sm:text-right">{children}</div>
    </div>
  );
}
...
function RunLogStatus({ entry }: { entry: WorkspaceRunLogEntry }) {
  if (entry.status === "next") {
    return (
      <span className="text-[15px] font-normal leading-[1.5] text-[#060B27]">
        Next run
      </span>
    );
  }
  ...
  return (
    <span className="flex items-center gap-2">
      ...
      <span className="flex size-4 items-center justify-center rounded-full bg-[#C65A66]">
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex flex-col gap-7">
          <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            ...
            <button
              ...
              className="inline-flex h-10 w-full shrink-0 items-center justify-center gap-2 rounded-[32px] border border-[#B3B8BD] px-3.5 text-[14px] font-medium leading-[1.24] text-[#010309] transition-colors hover:bg-[#F0F2F5] sm:w-auto"
            >
              Go to Analyst page
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              {info.runLog.map((entry) => (
                ...
                >
                  <p className="text-[15px] font-normal leading-[1.25] text-[#7E8185]">
                    {entry.label}
                  </p>
        ...
        <DialogClose className="absolute right-4 top-4 flex size-8 items-center justify-center rounded-full text-[#65686B] transition-colors hover:bg-[#F0F2F5] hover:text-[#353638]">
